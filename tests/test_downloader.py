...
class DownloaderTestCase(unittest.TestCase):
    def test_download_google(self):
        ...
        try:
            self.assertIsNotNone(re.match(r"Google_[0-9.]+_[0-9]+x0w.(jpeg|png|webp)", contain_google[0]))
        except AssertionError as e:
            print(contain_google)
            raise e
        finally:
            # cleanup
    ...
    def test_download_staples(self):
        ...
        try:
            self.assertIsNotNone(re.match(r"Staples.*?_[0-9.]+_[0-9]+x0w.(jpeg|png|webp)", contain_staples[0]))
        except AssertionError as e:
            print(contain_staples)
            raise e
        finally:
            # cleanup
